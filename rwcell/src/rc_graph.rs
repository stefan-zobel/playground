use crate::rw_cell::{RwCell, RwRef, RwRefMut};
use std::fmt;
...
type Link<T> = Option<Rc<RwCell<Node<T>>>>;
type WeakLink<T> = Option<Weak<RwCell<Node<T>>>>;
...
impl<T> NodeRef<T> {
    ...
    pub fn new(val: T) -> NodeRef<T> {
        NodeRef(Rc::new(RwCell::new(Node {
            parent: None,
            first_child: None,
            last_child: None,
    ...
    pub fn parent(&self) -> Option<NodeRef<T>> {
        Some(NodeRef(try_opt!(try_opt!(self
            .0
            .borrow_panic()
            .parent
            .as_ref())
    ...
    pub fn first_child(&self) -> Option<NodeRef<T>> {
        Some(NodeRef(
            try_opt!(self.0.borrow_panic().first_child.as_ref()).clone(),
        ))
    }
    ...
    pub fn next_sibling(&self) -> Option<NodeRef<T>> {
        Some(NodeRef(
            try_opt!(self.0.borrow_panic().next_sibling.as_ref()).clone(),
        ))
    }
    ...
    pub fn detach(&self) {
        self.0.borrow_mut_panic().detach();
    }
    ...
    pub fn append(&self, new_child: NodeRef<T>) {
        let mut self_borrow = self.0.borrow_mut_panic();
        let mut last_child_opt = None;
        {
            let mut new_child_borrow = new_child.0.borrow_mut_panic();
        ...
        if let Some(last_child_strong) = last_child_opt {
            let mut last_child_borrow = last_child_strong.borrow_mut_panic();
        ...
        let mut self_borrow = self.0.borrow_mut_panic();
        {
            ...
            match self_borrow.first_child.take() {
                Some(first_child_strong) => {
                    {
                        let mut first_child_borrow = first_child_strong.borrow_mut_panic();
    ...
    pub fn insert_after(&self, new_sibling: NodeRef<T>) {
        let mut self_borrow = self.0.borrow_mut_panic();
        {
            let mut new_sibling_borrow = new_sibling.0.borrow_mut_panic();
            ...
            match self_borrow.next_sibling.take() {
                Some(next_sibling_strong) => {
                    {
                        let mut next_sibling_borrow = next_sibling_strong.borrow_mut_panic();
                        debug_assert!({
                ...
                None => {
                    if let Some(parent_ref) = self_borrow.parent.as_ref() {
                        if let Some(parent_strong) = parent_ref.upgrade() {
                            let mut parent_borrow = parent_strong.borrow_mut_panic();
    ...
    pub fn insert_before(&self, new_sibling: NodeRef<T>) {
        ...
        if let Some(previous_sibling_strong) = previous_sibling_opt {
            let mut previous_sibling_borrow = previous_sibling_strong.borrow_mut_panic();
            debug_assert!({
...
impl<T> Node<T> {
    ...
    fn detach(&mut self) {
        ...
        if let Some(next_sibling_ref) = next_sibling_strong.as_ref() {
            let mut next_sibling_borrow = next_sibling_ref.borrow_mut_panic();
